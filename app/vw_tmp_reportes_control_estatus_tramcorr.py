...
def index(request):
    ...
    return render(request, 'app/cliente/tmp_reportes_control/index_estatus_tramcorr.html', {
...
def new(request):
    ...
    if 'POST' == request.method and frm.is_valid():
        ...
        obj.save()
        return HttpResponseRedirect(reverse('tmpestatustramcorr_see', kwargs={
            'pk': obj.pk
        }))
...
def see(request, pk):
    ...
    if usuario.has_perm_or_has_perm_child(
        ...
        toolbar.append({
            'type': 'link_pk',
            'view': 'tmpestatustramcorr_update',
    ...
    if usuario.has_perm_or_has_perm_child(
        ...
        toolbar.append({
            'type': 'link_pk_del',
            'view': 'tmpestatustramcorr_delete',
...
def update(request, pk):
    ...
    if "POST" == request.method and frm.is_valid():
        ...
        return HttpResponseRedirect(reverse(
            'tmpestatustramcorr_see', kwargs={'pk': obj.pk}))
    return render(request, 'global/form.html', {
        'menu_main': usuario.main_menu_struct(),
        'titulo': 'Estatus',
        'titulo_descripcion': obj,
